...
def _get_nearby_factories(latitude, longitude, radius, display_status=None):
    ...
    documents_query = Document.objects.filter(display_status=display_status)
    prefetch_query = Prefetch("documents", queryset=documents_query)
    ids = Factory.objects.prefetch_related(prefetch_query).only("id").filter(point__distance_lte=(pnt, D(km=radius)))
