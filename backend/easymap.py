...
def get_point_city(sess, x, y):
    ...
    if resp.status_code != requests.codes.ok:
        raise WebRequestError("Failed getting city code",
...
def get_token(sess):
    ...
    if resp.status_code != requests.codes.ok:
        raise WebRequestError("Failed getting token",
                resp.status_code, resp.text)
    token = dict([
        (m.group(1), m.group(2))
        for m in token_re.finditer(resp.text)
        ])
...
def get_door_info(sess, x, y, city, token):
    ...
    if resp.status_code != requests.codes.ok:
        raise WebRequestError("Failed getting door info",
                resp.status_code, resp.text)
    try:
        return resp.json()
    except:
